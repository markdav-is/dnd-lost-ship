[Campaign](../../Campaign.md) / [The Party](../The-Party.md) / Musty Jo Mustang <!-- wikidown:breadcrumb -->

# Musty-Jo Mustang

Centaur Fighter 5 / Paladin 3 (Outlander). Player: yourthighness69. Sheet: `assets/yourthighness69_111941481 (1).pdf`. Appears in session notes as "Musty Joe."

## Stats

| STR | DEX | CON | INT | WIS | CHA | AC | HP | Speed |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :--- |
| 18 (+4) | 13 (+1) | 12 (+1) | 9 (−1) | 14 (+2) | 13 (+1) | 16 | 60 | 40 ft |

- **Immunities: disease** (paladin) — and **critical hits** (listed defense). Disease immunity matters enormously against russet mold infection; the DM should rule explicitly whether the mold counts as disease.
- **Key skills:** Athletics +7, Perception +5 (passive 15), Survival +5, Intimidation +4.

## Combat

- **Flail of Tiamat** (+7, 1d8+4 +5d4 acid/cold/fire/lightning/poison), **Flame Tongue Longsword** (+7, +2d6 fire), longbow, hooves (+7, 1d4+4), Charge.
- **Lay on Hands** (15 HP pool), **Divine Sense**, **Channel Divinity** (1/short rest), **Superiority Dice** (4/short rest), **Second Wind / Action Surge**.

## DM hooks

- Was **grappled, stunned, and dominated repeatedly** by the mind flayers in the Level 2 theater — a grudge worth invoking.
- A centaur in ship corridors and drop tubes: mobility complications are a recurring (and fun) constraint — the wheely sleds and hoversled are his friends.

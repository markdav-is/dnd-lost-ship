[Home](../../Home.md) / [Campaign](../../Campaign.md) / [The Party](../The-Party.md) / Musty Jo Mustang <!-- wikidown:breadcrumb -->

# Musty-Jo Mustang

Centaur Fighter 6 / Paladin 5 (Outlander). Player: yourthighness69. Sheet: `assets/Musty-Jo Mustang.pdf` (2026-08-05). Appears in session notes as "Musty Joe."

## Stats

| STR | DEX | CON | INT | WIS | CHA | AC | HP | Speed |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :--- |
| 18 (+4) | 13 (+1) | 12 (+1) | 9 (−1) | 14 (+2) | 13 (+1) | 16 | 81 | 40 ft |

- **Immunities: disease** (paladin) — and **critical hits** (listed defense). Disease immunity matters enormously against russet mold infection; the DM should rule explicitly whether the mold counts as disease.
- **Key skills:** Athletics +8, Perception +6 (passive 16), Survival +6, Intimidation +5.
- **Languages:** Common, Elvish, Sylvan. **Tools:** cartographer's tools, flute.

## Combat

- **Extra Attack** (Fighter 6 and Paladin 5 both bring it). **Flail of Tiamat** (+8, 1d8+4 +5d4 acid/cold/fire/lightning/poison), **Flame Tongue Longsword** (+8, +2d6 fire), **Laser Rifle** (3d8+1 radiant — new), longbow, hooves, Charge.
- **Lay on Hands** (25 HP pool), **Divine Sense**, **Channel Divinity** (1/short rest), **Superiority Dice** (4/short rest), **Second Wind / Action Surge**.
- **Equine Build:** counts as one size larger for carrying capacity and push/drag — this is why his lift numbers below run so far ahead of the rest of the party.

## Equipment

**Currency:** 2,603 gp, 166 ep, 20 sp. **Weight carried:** 276.5 lb / **Encumbered at:** 540 lb / **Push, drag, lift:** 1080 lb (all per the 2026-08-05 sheet).

| Item | Qty | Weight |
| :--- | :-: | :-: |
| Bag of Beans | 1 | 0.5 lb |
| Folding Boat | 1 | 4 lb |
| Immovable Rod | 1 | — |
| Adamantine Chain Mail | 1 | — |
| Flame Tongue Longsword *(attuned)* | 1 | 3 lb |
| Potion of Healing (Greater) | 1 | 0.5 lb |
| Potion of Fire Resistance | 1 | — |
| Longbow, +1 | 1 | 2 lb |
| Potion of Fire Breath | 1 | — |
| Scroll of Protection | 1 | — |
| Armblade (Longsword) | 1 | 3 lb |
| Arrow of Electric Sting, +1 | 3 | 2 lb each |
| Arrow of Fire | 7 | — |
| Longbow (mundane, spare) | 1 | 2 lb |
| Backpack | 1 | 5 lb |
| Oil (flask) | 10 | 10 lb |
| Chain Mail (mundane, spare) | 1 | 55 lb |
| Shortbow | 1 | 2 lb |
| Lantern, Hooded | 1 | 2 lb |
| Diamond | 1 | — |
| Arrows, Walloping | 9 | — |
| Ball Bearings (bag of 1,000) | 1,000 | 2 lb |
| Flail of Tiamat | 1 | 2 lb |
| Silver Dragon Ball | 1 | — |
| Diamonds/gems (x3,000) | 1 | — |
| Potion of Healing | 3 | 0.5 lb |
| Android Gizmo (took a picture — eyeball lookin thing) | 1 | — |
| Belt of Flight | 1 | — |
| Laser Rifle | 1 | 7 lb |
| Geiger Counter | 1 | — |
| Juicer | 1 | — |
| Blender | 1 | — |
| Crowbar | 1 | 5 lb |
| Hammer | 1 | 3 lb |
| Piton | 10 | 2.5 lb |
| Rations (1 day) | 10 | 20 lb |
| Rope, Hempen (50 feet) | 1 | 10 lb |
| Tinderbox | 1 | 1 lb |
| Torch | 10 | 10 lb |
| Waterskin | 1 | 5 lb |
| Ancient Coin | 15 | — |
| Shart | 13 | — |
| Catnip | 1 | — |

## From the sheet

- **Appearance:** male, 23, 7 ft, 1,200 lb; tan skin, brown eyes, blond hair. Neutral Good.
- **Personality:** *Means well but not that smart. Very sweet and gentle except when those close to him are at risk. Then he go cray.*
- **Ideal:** *Old traditions, new traditions — basically believes whatever people tell him.*
- **Bond:** *Family, community; OK with people he trusts riding him.*
- **Flaw:** *Oblivious, trusting, gullible.*
- **Backstory field:** empty on the sheet. Outlander background; the rest is the player's to tell.

The **Silver Dragon Ball** in his pack is one of the dragon orbs from the previous campaign — which may make Musty-Jo the unconfirmed partner of the Silver/White Dragon that lives at Airlantis ([The Dragons](../../World/The-Dragons.md)). Worth confirming at the table.

## DM hooks

- Was **grappled, stunned, and dominated repeatedly** by the mind flayers in the Level 2 theater — a grudge worth invoking.
- A centaur in ship corridors and drop tubes: mobility complications are a recurring (and fun) constraint — the wheely sleds and hoversled are his friends.
- The [Counteragent](../../Game-Mechanics/The-Counteragent.md) syringe-lance build was written for him: Charge, pin, auto-dose. The party's cavalry is also its ambulance.
- **A Juicer, a Blender, and an Android Gizmo** are riding in his pack with no explanation yet — scavenged ship tech nobody's asked him about.
- **"Believes whatever people tell him."** On Aerun, where every merchant house and Guild factor lies for a living, that ideal is a hook with a barb on it. Also: the catnip is for someone specific.
